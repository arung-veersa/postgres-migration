...
WHERE 
  NOT EXISTS (
    SELECT 
      1 
    FROM 
      CONFLICTREPORT."PUBLIC".LOG_HISTORY AS LHI 
    WHERE 
      LHI."CONID" = T2.ID 
      AND LHI."LogTypeFlag" = 'Inserted'
  );
-- Now insert the log history 
values 
  using 
UNION ALL 
  for each field INSERT INTO CONFLICTREPORT."PUBLIC".LOG_HISTORY_VALUES (
    ...
    SELECT 
      ...
      TO_CHAR(
        T3."FlagForReviewDate", 'YYYY-MM-DD HH24:MI:SS'
    ...
    FROM 
      CONFLICTREPORT."PUBLIC".LOG_HISTORY LH 
    ...
    WHERE 
      LH."LogTypeFlag" = 'Inserted' 
      AND NOT EXISTS (
        SELECT 
          1 
        FROM 
          CONFLICTREPORT."PUBLIC".LOG_HISTORY_VALUES AS LHV 
        WHERE 
          (LHV."LHID" = LH.ID)
      )
  ), 
