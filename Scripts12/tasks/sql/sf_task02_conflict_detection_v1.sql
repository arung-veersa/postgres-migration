-- Task 02: Conflict Detection with CTE Optimization
-- Detects visit conflicts across providers for same caregiver (SSN) on same date
-- Optimized with CTEs to calculate ST_DISTANCE only once (vs 5+ times in original)
--
...
--   {excluded_ssns} - Comma-separated quoted list of excluded SSNs
...
--   {lookback_hours} - Hours for updated timestamp filter (default: 36)
--   {extra_distance_per} - ExtraDistancePer value from settings table (default: 100)
...
base_visits AS (
  ...
  WHERE 
    ...
    AND CR1."Visit Updated Timestamp" >= DATEADD(HOUR, -{lookback_hours}, GETDATE())
    AND CR1."Provider Id" NOT IN ({excluded_agencies})
    AND TRIM(CAR."SSN") NOT IN ({excluded_ssns})
),
