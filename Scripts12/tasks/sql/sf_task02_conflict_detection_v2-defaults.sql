-- ============================================================================
-- EXECUTABLE TEST QUERY: Asymmetric Join with Default Values
-- ============================================================================
-- 
-- PURPOSE: Test asymmetric join query performance directly in Snowflake
-- GENERATED BY: generate_test_query.py using actual query_builder.py
-- EXPECTED RUNTIME: Unknown - causing Lambda timeout (>15 mins)
-- 
-- This query has ALL placeholders replaced with default values from config.json
-- and typical reference data. You can execute this directly in DBeaver against
-- Snowflake to diagnose the performance bottleneck.
--
-- DEFAULT VALUES USED:
--   lookback_years: 2
--   lookforward_days: 45  
--   lookback_hours: 32
--   excluded_agencies: '1005', '9996', '9997' (typical test values)
--   extra_distance_per: 1.25
--   Database: ANALYTICS.BI
--
-- NOTES:
--   - Excluded SSNs filtering is DISABLED (as in the failing run)
--   - MPH lookup data hardcoded (4 ranges: 0-5mi, 5-10mi, 10-20mi, 20+mi)
--   - This is the ASYMMETRIC JOIN version (enable_asymmetric_join=true)
--
-- ⚠️  PERFORMANCE WARNING:
--   In asymmetric mode, base_visits has NO 32-hour timestamp filter!
--   This causes it to process the ENTIRE 2-year window before delta_visits
--   can filter it, potentially scanning hundreds of millions of rows.
--
-- ============================================================================
...
--   ANALYTICS, BI - Snowflake analytics database/schema
--   '1005','9996','9997' - Comma-separated quoted list of excluded provider IDs
--   2 - Years in past for visit date filter (default: 2)
--   45 - Days in future for visit date filter (default: 45)
--   32 - Hours for updated timestamp filter (default: 32)
--   1.25 - ExtraDistancePer value from settings table (default: 1.25)
...
mph_data AS (
  SELECT 1 AS "From", 5 AS "To", 15 AS "AverageMilesPerHour"
  UNION ALL
  SELECT 5 AS "From", 10 AS "To", 25 AS "AverageMilesPerHour"
  UNION ALL
  SELECT 10 AS "From", 20 AS "To", 35 AS "AverageMilesPerHour"
  UNION ALL
  SELECT 20 AS "From", 999999 AS "To", 45 AS "AverageMilesPerHour"
),
...
base_visits AS (
  ...
  WHERE 
    ...
    AND CR1."Provider Id" NOT IN ('1005','9996','9997')
    
),

-- CTE 2: Delta visits - visits updated in the last 32 hours
-- Simple filter on base_visits - no code duplication!
delta_visits AS (
  SELECT * 
  FROM base_visits
  WHERE "LastUpdatedDate" >= DATEADD(HOUR, -32, GETDATE())
),
...
conflict_pairs AS (
  -- Direction 1: Delta visits joined with all visits
  SELECT 
    V1."CONFLICTID",
    V1."SSN",
...
-- 
-- 1. To test just the base_visits CTE (check row count):
--    SELECT COUNT(*) FROM base_visits;
--    Expected: MILLIONS of rows (entire 2-year window, NO 32-hour filter)
--
-- 2. To test delta_visits CTE (check row count):
--    SELECT COUNT(*) FROM delta_visits;
--    Expected: Thousands of rows (last 32 hours only)
--
-- 3. To test delta_conflict_keys CTE (check unique combos):
--    SELECT COUNT(DISTINCT "VisitDate", "SSN") FROM delta_conflict_keys;
--
-- 4. To test all_visits CTE performance (the suspected bottleneck):
--    SELECT COUNT(*) FROM all_visits;
--    This is the likely slow step: filtering base_visits by IN subquery
--
-- 5. To see query execution plan:
--    Click "Explain" button in DBeaver before running
--    Look for "Scan" operations on large datasets without filters
--
-- 6. To monitor query progress:
--    Run: SELECT * FROM TABLE(INFORMATION_SCHEMA.QUERY_HISTORY()) 
