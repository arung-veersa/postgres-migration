...
SET
    ...
    "ETATravelMinutes"      = ALLDATA."ETATravelMinutes",
    ...
    FROM
        ...
        (
            ...
            LEFT JOIN {conflict_schema}.conflictvisitmaps AS CVM1
               ...
               AND CVM1."AppVisitID"::varchar = CR1."Application Visit Id"::varchar
            ...
            WHERE CR1."Visit Date"::date
              ...
              AND {chunk_filter_inner}  -- ⭐ INNER FILTER: Only scan rows for this chunk (prevents full table scan)
        ) AS V1
    LEFT JOIN
...
WHERE
    (
        (CVM."VisitID"::varchar = ALLDATA."VisitID"::varchar AND CVM."ConVisitID"::varchar = ALLDATA."ConVisitID"::varchar)
        OR
        (CVM."VisitID"::varchar = ALLDATA."VisitID"::varchar AND CVM."ConVisitID" IS NULL AND ALLDATA."ConVisitID" IS NULL)
    )
