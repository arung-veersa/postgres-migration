-- ============================================================================
-- TASK_02+03: Conflict Detection & Sync (CONSOLIDATED MERGE - OPTIMIZED)
-- ============================================================================
-- Purpose:
--   Detect and sync conflicts between visits using 7 conflict rules + InService:
...
--     8. InService flag (visit overlaps with caregiver in-service training)
--
--   This MERGE replaces:
--     - TASK_02_0 (7 conflict rule updates)
--     - TASK_02_1 (InServiceFlag updates)
--     - TASK_03_0, TASK_03_2 (inserts for 7 conflict rules + InService)
--
--   Note: PTO conflict logic is NOT included in this script.
--   PTO conflicts will be handled separately in a future script.
--
-- OPTIMIZATION NOTES:
--   This version reduces code duplication by ~290 lines through:
--   - Single base_visits CTE (eliminates V1/V2 duplication)
--   - Reusable patient_addresses CTE (eliminates 4 duplicate subqueries)
--   - Pre-computed time gap calculations
--   - Cleaner structure with identical functionality
--
-- Prerequisites:
--   Run functions/distance_functions.sql ONCE before first execution
--   (Creates: calculate_distance_miles, calculate_conflict_distance, calculate_travel_time_minutes)
--
...
USING (
    ...
    patient_addresses AS (
        SELECT 
            ...
            "Longitude" AS "Provider_Longitude",
            "Latitude" AS "Provider_Latitude",
            ROW_NUMBER() OVER (
    ...
    base_visits AS (
        SELECT DISTINCT 
            ...
            CASE 
                ...
                THEN SPLIT_PART(CR1."Call Out GPS Coordinates", ',', 2)::numeric
                WHEN CR1."Call In GPS Coordinates" IS NOT NULL 
                     AND CR1."Call In GPS Coordinates" != ',' 
                THEN SPLIT_PART(CR1."Call In GPS Coordinates", ',', 2)::numeric
                ELSE DPAD_P."Provider_Longitude" 
            END AS "Longitude",
            CASE 
                WHEN CR1."Call Out GPS Coordinates" IS NOT NULL 
                     AND CR1."Call Out GPS Coordinates" != ',' 
                THEN SPLIT_PART(CR1."Call Out GPS Coordinates", ',', 1)::numeric
                WHEN CR1."Call In GPS Coordinates" IS NOT NULL 
                     AND CR1."Call In GPS Coordinates" != ',' 
                THEN SPLIT_PART(CR1."Call In GPS Coordinates", ',', 1)::numeric
            ...
            END AS "ContractType",
            -- In-service dates
            FCI."Inservice start date" AS "InserviceStartDate",
            FCI."Inservice end date" AS "InserviceEndDate"
        ...
        LEFT JOIN :analytics_schema.factcaregiverinservice AS FCI
            ON FCI."Caregiver Id" = CR1."Caregiver Id"
            AND FCI."Office Id" = CR1."Office Id"
            AND CR1."Visit Date"::date BETWEEN FCI."Inservice start date"::date AND FCI."Inservice end date"::date
    ...
    SELECT DISTINCT
        ...
        -- RULE 7: Distance-based conflict (travel time > gap between visits)
        CASE 
            WHEN V1."ProviderID" != V2."ProviderID"
        ...
        -- InService Flag: Marks when visit overlaps with in-service training
        CASE
            WHEN V1."ProviderID" != V2."ProviderID"
                AND (
                    (V1."VisitStartTime" IS NOT NULL AND V1."VisitEndTime" IS NOT NULL AND V2."InserviceStartDate" IS NOT NULL AND V2."InserviceEndDate" IS NOT NULL
                     AND V1."VisitStartTime" <= V2."InserviceEndDate" AND V1."VisitEndTime" >= V2."InserviceStartDate")
                    OR
                    (V2."VisitStartTime" IS NOT NULL AND V2."VisitEndTime" IS NOT NULL AND V1."InserviceStartDate" IS NOT NULL AND V1."InserviceEndDate" IS NOT NULL
                     AND V2."VisitStartTime" <= V1."InserviceEndDate" AND V2."VisitEndTime" >= V1."InserviceStartDate")
                )
            THEN 'Y'
            ELSE 'N'
        END AS "InServiceFlag",
...
-- ============================================================================
-- TASK_02+03 Complete (OPTIMIZED VERSION)
-- ============================================================================
-- Expected outcome:
...
--   - Reduced code duplication by ~290 lines
--   - Single base_visits CTE eliminates V1/V2 duplication
--   - Reusable patient_addresses CTE (used 4x)
