...
SET 
  ...
  CVM."InServiceFlag" = CASE WHEN CVM."InServiceFlag" = ' N ' THEN ALLDATA."InServiceFlag" ELSE CVM."InServiceFlag" END, 
  ...
  CVM."PTOStartDate" = ALLDATA."PTOStartDate", 
  CVM."PTOEndDate" = ALLDATA."PTOEndDate", 
  CVM."ConPTOStartDate" = ALLDATA."ConPTOStartDate", 
  CVM."ConPTOEndDate" = ALLDATA."ConPTOEndDate" 
FROM 
  (
    SELECT 
    ...
    FROM 
      (
        SELECT 
          ...
          CAST(
            FCA."Caregiver Vacation Id" AS VARCHAR
          ) AS "PTOVacationID", 
        ...
        FROM 
          ...
          LEFT JOIN CONFLICTREPORT."PUBLIC".CONFLICTVISITMAPS AS CVM1 ON CVM1."VisitID" = CR1."Visit Id" 
          ...
          LEFT JOIN ANALYTICS.BI.FACTCAREGIVERABSENCE AS FCA ON FCA."Global Caregiver Id" = CR1."Caregiver Id" 
          AND FCA."Provider Id" = CR1."Provider Id" 
          AND (
            CR1."Visit Start Time" IS NOT NULL 
            AND CR1."Visit End Time" IS NOT NULL 
            AND CAST(CR1."Visit Start Time" AS DATE) <= CAST(FCA."End Date" AS DATE) 
            AND CAST(CR1."Visit End Time" AS DATE) >= CAST(FCA."Start Date" AS DATE)
          ) 
        WHERE 
          CR1."Is Missed" = FALSE 
          AND FCA."Caregiver Vacation Id" IS NULL 
          AND DATE(CR1."Visit Date") BETWEEN DATE(
            DATEADD(
              year, 
              -2, 
              GETDATE()
          ...
          AND CR1."Provider Id" NOT IN(
            SELECT 
              "ProviderID" AS PAID 
            FROM 
              CONFLICTREPORT."PUBLIC".EXCLUDED_AGENCY
          ) 
          AND NOT EXISTS (
            SELECT 
              1 
            FROM 
              CONFLICTREPORT."PUBLIC".EXCLUDED_SSN AS SSN 
            WHERE 
      ...
      INNER JOIN (
        ...
        FROM 
          ...
          INNER JOIN ANALYTICS.BI.DIMCAREGIVER AS CAR ON CAR."Caregiver Id" = FCA."Global Caregiver Id" 
          AND TRIM(CAR."SSN") IS NOT NULL 
          AND TRIM(CAR."SSN")!= '' 
          INNER JOIN ANALYTICS.BI.DIMPROVIDER AS DPR ON DPR."Provider Id" = FCA."Provider Id" 
          AND DPR."Is Active" = TRUE 
          AND DPR."Is Demo" = FALSE 
          LEFT JOIN ANALYTICS.BI.DIMOFFICE AS DOF ON DOF."Office Id" = FCA."Office Id" 
          AND DOF."Is Active" = TRUE 
          LEFT JOIN CONFLICTREPORT."PUBLIC".CONFLICTVISITMAPS AS CVM1 ON CVM1."VisitID" = MD5(
            CONCAT(
              ' P ', 
              CAST(
                FCA."Caregiver Vacation Id" AS VARCHAR
          ...
          AND CVM1."CONFLICTID" IS NOT NULL 
        WHERE 
          CAST(FCA."Start Date" AS DATE) BETWEEN DATE(
            DATEADD(
              year, 
              -2, 
              GETDATE()
      ...
      ) AS V2 ON V1.SSN = V2.SSN 
      AND (
        CAST(V1."VisitStartTime" AS DATE) <= CAST(V2."PTOEndDate" AS DATE) 
        AND CAST(V1."VisitEndTime" AS DATE) >= CAST(V2."PTOStartDate" AS DATE)
      ) 
      AND V1."ProviderID" IS NOT NULL 
      AND V2."ProviderID" != V1."ProviderID" 
      AND V2."PTOVacationID" IS NOT NULL 
      AND V1."PTOVacationID" IS NULL
  ) AS ALLDATA 
WHERE 
  CVM."VisitID" = ALLDATA."VisitID" 
  ...
  AND CVM."ConPTOStartDate" IS NOT NULL 
  AND CVM."ConPTOEndDate" IS NOT NULL 
    ...
    FROM 
      ...
      INNER JOIN (
        SELECT 
          ...
          AND CR1."Call In GPS Coordinates" != ', 
          ' THEN REPLACE(
            SPLIT(
              CR1."Call In GPS Coordinates", ', 
              '
            ) [1], 
            ' "', CAST(NULL AS NUMBER)) WHEN CR1." Call Out GPS Coordinates " IS NOT NULL AND CR1." Call Out GPS Coordinates " != ',' THEN REPLACE(SPLIT(CR1." Call Out GPS Coordinates ", ',')[1], '" ', 
            CAST(NULL AS NUMBER)
          ) ELSE DPAD_P."Provider_Longitude" END AS "Longitude", 
          CASE WHEN CR1."Call In GPS Coordinates" IS NOT NULL 
          AND CR1."Call In GPS Coordinates" != ', 
          ' THEN REPLACE(
            ...
            ' "', CAST(NULL AS NUMBER)) WHEN CR1." Call Out GPS Coordinates " IS NOT NULL AND CR1." Call Out GPS Coordinates " != ',' THEN REPLACE(SPLIT(CR1." Call Out GPS Coordinates ", ',')[0], '" ', 
            CAST(NULL AS NUMBER)
          ) ELSE DPAD_P."Provider_Latitude" END AS "Latitude", 
          CR1."Payer Id" as "PayerID", 
          CR1."Application Payer Id" as "AppPayerID", 
      ...
      ) AS V2 ON V1."VisitID" != V2."VisitID" 
      AND V1.SSN = V2.SSN 
      AND (
        CAST(V2."VisitStartTime" AS DATE) <= CAST(V1."PTOEndDate" AS DATE) 
        AND CAST(V2."VisitEndTime" AS DATE) >= CAST(V1."PTOStartDate" AS DATE)
      ) 
      AND V2."ProviderID" IS NOT NULL 
      AND V1."ProviderID" != V2."ProviderID" 
      AND V1."PTOVacationID" IS NOT NULL 
      AND V2."PTOVacationID" IS NULL
  ) AS ALLDATA 
WHERE 
  CVM."VisitID" = ALLDATA."VisitID" 
  AND CVM."ConVisitID" = ALLDATA."ConVisitID" 
  AND CVM."PTOStartDate" IS NOT NULL 
  AND CVM."PTOEndDate" IS NOT NULL 
