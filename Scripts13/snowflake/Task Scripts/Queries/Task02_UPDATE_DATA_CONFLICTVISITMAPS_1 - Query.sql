...
SET 
  ...
  CVM."InserviceStartDate" = ALLDATA."InserviceStartDate", 
  CVM."InserviceEndDate" = ALLDATA."InserviceEndDate", 
  CVM."ConInserviceStartDate" = ALLDATA."ConInserviceStartDate", 
    ...
    FROM 
      (
        SELECT 
          ...
          CAST(
            FCS."Inservice start date" AS timestamp
          ) AS "InserviceStartDate", 
          CAST(
            FCS."Inservice end date" AS timestamp
          ) AS "InserviceEndDate", 
          CAST(
            FCS."Application Caregiver Inservice Id" AS VARCHAR
          ) AS "AppCaregiverInserviceID", 
        ...
        FROM 
          ...
          LEFT JOIN ANALYTICS.BI.FACTCAREGIVERINSERVICE AS FCS ON FCS."Caregiver Id" = CR1."Caregiver Id" 
      ...
      INNER JOIN (
        SELECT 
          ...
          CAST(
            FCS."Inservice start date" AS DATE
          ) AS "VisitDate", 
        ...
        FROM 
          ...
          INNER JOIN ANALYTICS.BI.DIMCAREGIVER AS CAR ON CAR."Caregiver Id" = FCS."Caregiver Id" 
          AND TRIM(CAR."SSN") IS NOT NULL 
          AND TRIM(CAR."SSN")!= '' 
          INNER JOIN ANALYTICS.BI.DIMPROVIDER AS DPR ON DPR."Provider Id" = FCS."Provider Id" 
          AND DPR."Is Active" = TRUE 
          AND DPR."Is Demo" = FALSE 
          LEFT JOIN ANALYTICS.BI.DIMOFFICE AS DOF ON DOF."Office Id" = FCS."Office Id" 
          AND DOF."Is Active" = TRUE 
      ...
      AND V1.SSN = V2.SSN 
      AND (
        CAST(V1."VisitStartTime" AS timestamp) <= CAST(
          V2."InserviceEndDate" AS timestamp
        ) 
        AND CAST(V1."VisitEndTime" AS timestamp) >= CAST(
          V2."InserviceStartDate" AS timestamp
        )
      ) 
      AND V1."ProviderID" IS NOT NULL 
      AND V2."ProviderID" != V1."ProviderID" 
      AND V2."AppCaregiverInserviceID" IS NOT NULL 
      AND V1."AppCaregiverInserviceID" IS NULL
  ) AS ALLDATA 
WHERE 
  CVM."VisitID" = ALLDATA."VisitID" 
  ...
  AND CVM."ConInserviceStartDate" IS NOT NULL 
  AND CVM."ConInserviceEndDate" IS NOT NULL 
    ...
    FROM 
      ...
      INNER JOIN (
        SELECT 
          ...
          CAST(NULL AS STRING) "PTOStartDate", 
          CAST(NULL AS STRING) "PTOEndDate", 
          CAST(NULL AS STRING) "PTOVacationID", 
          DSC."Service Code Id" AS "ServiceCodeID", 
          DSC."Application Service Code Id" AS "AppServiceCodeID", 
          CR1."Bill Type" as "RateType", 
      ...
      AND V1.SSN = V2.SSN 
      AND (
        CAST(V2."VisitStartTime" AS timestamp) <= CAST(
          V1."InserviceEndDate" AS timestamp
        ) 
        AND CAST(V2."VisitEndTime" AS timestamp) >= CAST(
          V1."InserviceStartDate" AS timestamp
        )
      ) 
      AND V2."ProviderID" IS NOT NULL 
      AND V2."AppProviderID" IS NOT NULL 
      AND V1."ProviderID" != V2."ProviderID" 
      AND V1."AppCaregiverInserviceID" IS NOT NULL 
      AND V2."AppCaregiverInserviceID" IS NULL
  ) AS ALLDATA 
WHERE 
  CVM."VisitID" = ALLDATA."VisitID" 
  AND CVM."ConVisitID" = ALLDATA."ConVisitID" 
