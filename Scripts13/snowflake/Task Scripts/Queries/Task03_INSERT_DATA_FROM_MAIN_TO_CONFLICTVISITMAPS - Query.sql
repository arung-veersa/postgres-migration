...
SELECT 
  ...
  AND V1."VisitDate" >= CURRENT_DATE THEN 'Y' ELSE 'N' END AS "SameSchTimeFlag", 
  CASE WHEN V1."ProviderID" != V2."ProviderID" 
  ...
  AND V1."VisitDate" >= CURRENT_DATE THEN 'Y' ELSE 'N' END AS "SchOverAnotherSchTimeFlag", 
  CASE WHEN V1."ProviderID" != V2."ProviderID" 
  AND V1."VisitStartTime" IS NOT NULL 
  AND V1."VisitEndTime" IS NOT NULL 
  AND V2."VisitStartTime" IS NOT NULL 
  ...
  AND (
    (
      ...
      AND V2."P_PZipCode" IS NOT NULL 
      AND V1."P_PZipCode" != V2."P_PZipCode"
    ) 
    OR (
...
FROM 
  (
    ...
    WHERE 
      ...
      AND --CR1."Application Provider Id" IN (651, 891, 532, 7646) AND
      CR1."Provider Id" NOT IN(
        SELECT 
          "ProviderID" AS PAID 
        FROM 
  ...
  LEFT JOIN (
    SELECT 
      ...
      CAST(NULL AS TIMESTAMP) "InserviceEndDate", 
      CAST(NULL AS STRING) "AppCaregiverInserviceID", 
      CAST(NULL AS STRING) "FCSVisitID", 
      CAST(NULL AS STRING) "FCSAppVisitID", 
      CAST(NULL AS STRING) "FCAVisitID", 
      CAST(NULL AS STRING) "FCAAppVisitID", 
      CAST(NULL AS TIMESTAMP) "PTOStartDate", 
      CAST(NULL AS TIMESTAMP) "PTOEndDate", 
      CAST(NULL AS STRING) "PTOVacationID", 
      DSC."Service Code Id" AS "ServiceCodeID", 
  ...
  AND V1."VisitID" != V2."VisitID" CROSS 
  JOIN CONFLICTREPORT."PUBLIC"."SETTINGS" AS SETT 
  LEFT JOIN CONFLICTREPORT."PUBLIC".MPH AS MPH ON ROUND(
    (
      ST_DISTANCE(
        ST_MAKEPOINT(V1."Longitude", V1."Latitude"), 
        ST_MAKEPOINT(V2."Longitude", V2."Latitude")
