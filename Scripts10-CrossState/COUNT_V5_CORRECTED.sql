-- Summary: Total records to be deleted from both tables (V5 CORRECTED)
WITH 
...
-- Identify CONFLICTIDs where ALL rows are being deleted
...
-- Identify individual CONFLICTVISITMAPS rows to delete
ConflictVisitMapsToDelete AS (
    SELECT "ID", "CONFLICTID"
    FROM ConflictClassification
...
-- Final counts
SELECT 
    'Total CONFLICTVISITMAPS rows' AS "Category",
    (SELECT COUNT(*) FROM CONFLICTREPORT."PUBLIC".CONFLICTVISITMAPS) AS "Count"

UNION ALL

SELECT 
    'CONFLICTVISITMAPS rows to DELETE',
    (SELECT COUNT(*) FROM ConflictVisitMapsToDelete)

UNION ALL

SELECT 
    'CONFLICTVISITMAPS rows to KEEP',
...
SELECT 
    'Total unique CONFLICTIDs in CONFLICTVISITMAPS',
    (SELECT COUNT(DISTINCT "CONFLICTID") FROM CONFLICTREPORT."PUBLIC".CONFLICTVISITMAPS)

UNION ALL

SELECT 
    'Unique CONFLICTIDs with at least 1 row to DELETE',
    (SELECT COUNT(DISTINCT "CONFLICTID") FROM ConflictVisitMapsToDelete)

UNION ALL

SELECT 
    'Unique CONFLICTIDs with ALL rows to DELETE',
    (SELECT COUNT(*) FROM ConflictsToDelete)
...
SELECT 
    'Total CONFLICTS records',
    (SELECT COUNT(*) FROM CONFLICTREPORT."PUBLIC".CONFLICTS)

UNION ALL

SELECT 
    'CONFLICTS records to DELETE',
    (SELECT COUNT(*) FROM CONFLICTREPORT."PUBLIC".CONFLICTS 
     WHERE "CONFLICTID" IN (SELECT "CONFLICTID" FROM ConflictsToDelete))

UNION ALL

SELECT 
    'CONFLICTS records to KEEP',
    (SELECT COUNT(*) FROM CONFLICTREPORT."PUBLIC".CONFLICTS) - 
    (SELECT COUNT(*) FROM CONFLICTREPORT."PUBLIC".CONFLICTS 
     WHERE "CONFLICTID" IN (SELECT "CONFLICTID" FROM ConflictsToDelete));
