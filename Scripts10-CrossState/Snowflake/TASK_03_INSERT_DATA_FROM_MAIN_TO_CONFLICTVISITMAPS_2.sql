...
<parameter name="new_string">DPAD_PA."Zip Code" as "PA_PZipCode", DPAD_PA."County" as "PA_PCounty", CASE WHEN (CR1."Application Payer Id" = ''0'' AND CR1."Application Contract Id" != ''0'') THEN ''Internal'' WHEN (CR1."Application Payer Id" != ''0'' AND CR1."Application Contract Id" != ''0'') THEN ''UPR'' WHEN (CR1."Application Payer Id" != ''0'' AND CR1."Application Contract Id" = ''0'') THEN ''Payer'' END AS "ContractType", COALESCE(CASE WHEN UPPER(TRIM(DPAD_P."Address State")) = ''MISSISSIPPI'' THEN ''MS'' WHEN UPPER(TRIM(DPAD_P."Address State")) = ''NEW YORK'' THEN ''NY'' ELSE UPPER(NULLIF(TRIM(DPAD_P."Address State"), '''')) END, CASE WHEN UPPER(TRIM(DPAD_PA."Address State")) = ''MISSISSIPPI'' THEN ''MS'' WHEN UPPER(TRIM(DPAD_PA."Address State")) = ''NEW YORK'' THEN ''NY'' ELSE UPPER(NULLIF(TRIM(DPAD_PA."Address State"), '''')) END) AS "NormalizedState", NULLIF(TRIM(UPPER(DPR."Address State")), '''') AS "ProviderState" FROM ANALYTICS.BI.FACTVISITCALLPERFORMANCE_CR AS CR1 
       ...
       (
		...
		CAST(NULL AS STRING) "NormalizedState"
		FROM
